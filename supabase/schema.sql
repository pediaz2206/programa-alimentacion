...
--
-- `care_rel_update` decide QUIEN toca la fila y nunca decidio QUE columna toca
-- cada uno. Sin esto el profesional se concede el consentimiento solo, el
-- entrenador se asciende a nutricionista escribiendo su propio `rol`, y
-- cualquiera de los dos reapunta `patient_id` a un uuid ajeno.
--
-- RLS decide por fila; las columnas las decide este trigger. Los privilegios
-- de columna no sirven: son por rol de Postgres y todos los usuarios de la app
-- son el mismo rol `authenticated`.
